...
std:
    ...
    //////////////////SM算法验证阶段////////////////////////////
    bool ret = false;
    for (int i = 1; i <= 5 && !stopFlag.load(); i += 2)
    {
        ...
        case REQ_CERTI:
        {
            ...
            len = send(clientfd, request.c_str(), strlen(request.c_str()) + 1, 0);
            if (len == -1)
            {
                ret = false;
...
void readTaskHandler(int clientfd)
{
    ...
    for (;;)
    {
        ...
        else if (msgtype == RSP_ENCODE_TEST)
        {
            if (!SM_3(js))
            {
                stopFlag = false;
